...
if (onQueue == false) {
  ...
  if(api.getPlayerIds.length == 1) {
   api.setClientOptions(myId, {'middleTextLower': `You are now in the queue for a 1v1. Estimated wait time: Forever cuz u hav no friends 💀.`});
api.sendMessage(myId, "You are now in the queue for a 1v1. Estimated wait time: Forever cuz u hav no friends 💀.", {
    color: "green",});
  }else{
   api.setClientOptions(myId, {'middleTextLower': `You are now in the queue for a 1v1. Estimated wait time: ${(1/api.getPlayerIds.length*5)/60+1} - ${(1/api.getPlayerIds.length*5.5)/60+1} Seconds.`});
api.sendMessage(myId, "You are now in the queue for a 1v1. Estimated wait time: "+(1/api.getPlayerIds.length*5/60+1)+" - "+(1/api.getPlayerIds.length*5.5/60+1)+" Seconds.", {
    color: "green",}); 
}
